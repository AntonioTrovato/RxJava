...
public class MaybeMapOptionalTest extends RxJavaTest {
    ...
    public void empty() throws Throwable {
        ...
        Maybe.<Integer>empty().mapOptional(f).test().assertResult();
    ...
    public void error() throws Throwable {
        @SuppressWarnings("unchecked")
        Function<? super Integer, Optional<? extends Integer>> f = mock(Function.class);
        Maybe.<Integer>error(new TestException()).mapOptional(f).test().assertFailure(TestException.class);
        verify(f, never()).apply(any());
    }

    @Test
    public void mapperCrash() {
        Maybe.just(1).mapOptional(v -> {
