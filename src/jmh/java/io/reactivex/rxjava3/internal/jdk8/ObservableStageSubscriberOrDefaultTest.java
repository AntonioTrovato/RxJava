...
public class ObservableStageSubscriberOrDefaultTest extends RxJavaTest {
    ...
    public void firstJust() throws Exception {
        Integer v = Observable.just(1).firstStage(null).toCompletableFuture().get();
        assertEquals((Integer) 1, v);
    }

    @Test
    public void firstEmpty() throws Exception {
        Integer v = Observable.<Integer>empty().firstStage(2).toCompletableFuture().get();
    ...
    public void firstError() throws Exception {
        CompletableFuture<Integer> cf = Observable.<Integer>error(new TestException()).firstStage(null).toCompletableFuture();
        assertTrue(cf.isDone());
        assertTrue(cf.isCompletedExceptionally());
        assertFalse(cf.isCancelled());
        TestHelper.assertError(cf, TestException.class);
    ...
    public void singleJust() throws Exception {
        Integer v = Observable.just(1).singleStage(null).toCompletableFuture().get();
        assertEquals((Integer) 1, v);
    }

    @Test
    public void singleEmpty() throws Exception {
        Integer v = Observable.<Integer>empty().singleStage(2).toCompletableFuture().get();
        assertEquals((Integer) 2, v);
    }
    ...
    public void singleError() throws Exception {
        CompletableFuture<Integer> cf = Observable.<Integer>error(new TestException()).singleStage(null).toCompletableFuture();
        assertTrue(cf.isDone());
        assertTrue(cf.isCompletedExceptionally());
        assertFalse(cf.isCancelled());
        TestHelper.assertError(cf, TestException.class);
    ...
    public void lastJust() throws Exception {
        Integer v = Observable.just(1).lastStage(null).toCompletableFuture().get();
        assertEquals((Integer) 1, v);
    }

    @Test
    public void lastRange() throws Exception {
        Integer v = Observable.range(1, 5).lastStage(null).toCompletableFuture().get();
        assertEquals((Integer) 5, v);
    }

    @Test
    public void lastEmpty() throws Exception {
        Integer v = Observable.<Integer>empty().lastStage(2).toCompletableFuture().get();
        assertEquals((Integer) 2, v);
    }
    ...
    public void lastError() throws Exception {
        CompletableFuture<Integer> cf = Observable.<Integer>error(new TestException()).lastStage(null).toCompletableFuture();
        assertTrue(cf.isDone());
        assertTrue(cf.isCompletedExceptionally());
        assertFalse(cf.isCancelled());
        TestHelper.assertError(cf, TestException.class);
