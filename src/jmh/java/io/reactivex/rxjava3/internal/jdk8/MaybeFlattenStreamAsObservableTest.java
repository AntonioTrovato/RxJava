...
public class MaybeFlattenStreamAsObservableTest extends RxJavaTest {
    ...
    public void empty() throws Throwable {
        ...
        Maybe.<Integer>empty().flattenStreamAsObservable(f).test().assertResult();
    ...
    public void error() throws Throwable {
        @SuppressWarnings("unchecked")
        Function<? super Integer, Stream<? extends Integer>> f = mock(Function.class);
        Maybe.<Integer>error(new TestException()).flattenStreamAsObservable(f).test().assertFailure(TestException.class);
        verify(f, never()).apply(any());
    }

    @Test
    public void mapperCrash() {
        Maybe.just(1).flattenStreamAsObservable(v -> {
    ...
    public void fusedEmpty() {
        ...
        Maybe.just(1).flattenStreamAsObservable(v -> Stream.<Integer>of()).subscribe(to);
    ...
    public void fusedJust() {
        TestObserverEx<Integer> to = new TestObserverEx<>();
        to.setInitialFusionMode(QueueFuseable.ANY);
        Maybe.just(1).flattenStreamAsObservable(v -> Stream.<Integer>of(v)).subscribe(to);
    ...
    public void fusedMany() {
        TestObserverEx<Integer> to = new TestObserverEx<>();
        to.setInitialFusionMode(QueueFuseable.ANY);
        Maybe.just(1).flattenStreamAsObservable(v -> Stream.<Integer>of(v, v + 1, v + 2)).subscribe(to);
    ...
    public void fusedStreamAvailableLater() {
        ...
        ms.flattenStreamAsObservable(v -> Stream.<Integer>of(v, v + 1, v + 2)).subscribe(to);
    ...
    public void fused() throws Throwable {
        ...
        MaybeSubject<Integer> ms = MaybeSubject.create();
        ms.flattenStreamAsObservable(Stream::of).subscribe(new Observer<Integer>() {
    ...
    public void fused2() throws Throwable {
        AtomicReference<QueueDisposable<Integer>> qdr = new AtomicReference<>();
        MaybeSubject<Integer> ms = MaybeSubject.create();
        ms.flattenStreamAsObservable(v -> Stream.of(v, v + 1)).subscribe(new Observer<Integer>() {
    ...
    public void cancelAfterHasNextInDrain() {
        ...
        Maybe.just(1).flattenStreamAsObservable(v -> stream).subscribeWith(to).assertValuesOnly(1);
    }
    ...
    public void cancelAfterNextInDrain() {
        ...
        Maybe.just(1).flattenStreamAsObservable(v -> stream).subscribeWith(to).assertEmpty();
    }
    ...
    public void cancelSuccessRace() {
        for (int i = 0; i < TestHelper.RACE_LONG_LOOPS; i++) {
            MaybeSubject<Integer> ms = MaybeSubject.create();
            TestObserver<Integer> to = new TestObserver<>();
            ms.flattenStreamAsObservable(Stream::of).subscribe(to);
