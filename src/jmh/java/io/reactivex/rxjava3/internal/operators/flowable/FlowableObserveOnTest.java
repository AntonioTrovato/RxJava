...
public class FlowableObserveOnTest extends RxJavaTest {
    ...
    public void backpressureWithTakeAfterAndMultipleBatches() {
        // should be 4 batches == ((3*n)+1) items
    ...
    public void errorDelayed() {
        ...
        ts.assertNoErrors();
        ts.assertNotComplete();
        // requesting 2 doesn't switch to the error() source for some reason in concat.
        ts.request(3);
        s.advanceTimeBy(1, TimeUnit.SECONDS);
    ...
    public void conditionalConsumerFused() {
        ...
        ts.assertFuseable().assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertResult(2, 4);
    }
    ...
    public void conditionalConsumerFusedReject() {
        ...
        ts.assertFuseable().assertFusionMode(QueueFuseable.NONE).awaitDone(5, TimeUnit.SECONDS).assertResult(2, 4);
    }

    @Test
    public void requestOne() throws Exception {
        TestSubscriberEx<Integer> ts = Flowable.range(1, 5).observeOn(Schedulers.single()).to(TestHelper.<Integer>testSubscriber(1L));
        Thread.sleep(100);
    ...
    public void requestOneConditional() throws Exception {
        TestSubscriberEx<Integer> ts = Flowable.range(1, 5).observeOn(Schedulers.single()).filter(Functions.alwaysTrue()).to(TestHelper.<Integer>testSubscriber(1L));
        Thread.sleep(100);
        ts.assertSubscribed().assertValue(1).assertNoErrors().assertNotComplete();
    }
    ...
    public void outputFused() {
        ...
        ts.assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2, 3, 4, 5);
    }
    ...
    public void outputFusedReject() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<Integer>().setInitialFusionMode(QueueFuseable.SYNC);
        Flowable.range(1, 5).hide().observeOn(Schedulers.single()).subscribe(ts);
        ts.assertFusionMode(QueueFuseable.NONE).awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2, 3, 4, 5);
    }
    ...
    public void inputOutputAsyncFusedError() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<Integer>().setInitialFusionMode(QueueFuseable.ANY);
        UnicastProcessor<Integer> up = UnicastProcessor.create();
        up.observeOn(Schedulers.single()).subscribe(ts);
        up.onError(new TestException());
        ts.awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
        ts.assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
    }
    ...
    public void inputOutputAsyncFusedErrorDelayed() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<Integer>().setInitialFusionMode(QueueFuseable.ANY);
        UnicastProcessor<Integer> up = UnicastProcessor.create();
        up.observeOn(Schedulers.single(), true).subscribe(ts);
        up.onError(new TestException());
        ts.awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
        ts.assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
    }
    ...
    public void conditionalNormal() {
        ...
        }).take(250).to(TestHelper.<Integer>testConsumer()).awaitDone(5, TimeUnit.SECONDS).assertSubscribed().assertValueCount(250).assertNoErrors().assertComplete();
    }
    ...
    public void syncFusedCancelAfterRequestConditional() {
        ...
        Flowable.range(1, 3).observeOn(Schedulers.single()).filter(Functions.alwaysTrue()).subscribe(ts);
        ts.awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2);
    }
    ...
    public void syncFusedCancelAfterRequestConditional2() {
        final TestSubscriber<Integer> ts = new TestSubscriber<>(2L);
        Flowable.range(1, 2).observeOn(Schedulers.single()).filter(Functions.alwaysTrue()).subscribe(ts);
        ts.awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2);
    }
    ...
    public void nonFusedCancelAfterRequestConditional2() {
        final TestSubscriber<Integer> ts = new TestSubscriber<>(2L);
        Flowable.range(1, 2).hide().observeOn(Schedulers.single()).filter(Functions.alwaysTrue()).subscribe(ts);
        ts.awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2);
    }
    ...
    public void backFusedConditional() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<Integer>().setInitialFusionMode(QueueFuseable.ANY);
        Flowable.range(1, 100).hide().observeOn(ImmediateThinScheduler.INSTANCE).filter(Functions.alwaysTrue()).subscribe(ts);
        ts.assertFusionMode(QueueFuseable.ASYNC).assertValueCount(100).assertComplete().assertNoErrors();
    }

    @Test
    public void backFusedErrorConditional() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<Integer>().setInitialFusionMode(QueueFuseable.ANY);
        Flowable.<Integer>error(new TestException()).observeOn(ImmediateThinScheduler.INSTANCE).filter(Functions.alwaysTrue()).subscribe(ts);
        ts.assertFusionMode(QueueFuseable.ASYNC).assertFailure(TestException.class);
    }
    ...
    public void backFusedCancelConditional() {
        for (int i = 0; i < TestHelper.RACE_DEFAULT_LOOPS; i++) {
            ...
            Flowable.just(1).hide().observeOn(scheduler).filter(Functions.alwaysTrue()).subscribe(ts);
    ...
    static final class TestSubscriberFusedCanceling extends TestSubscriberEx<Integer> {
    ...
    public void workerNotDisposedPrematurelyNormalInNormalOutConditional() {
        DisposeTrackingScheduler s = new DisposeTrackingScheduler();
        Flowable.concat(Flowable.just(1).hide().observeOn(s).filter(Functions.alwaysTrue()), Flowable.just(2)).test().assertResult(1, 2);
        assertEquals(1, s.disposedCount.get());
    }
    ...
    public void workerNotDisposedPrematurelySyncInNormalOutConditional() {
        DisposeTrackingScheduler s = new DisposeTrackingScheduler();
        Flowable.concat(Flowable.just(1).observeOn(s).filter(Functions.alwaysTrue()), Flowable.just(2)).test().assertResult(1, 2);
        assertEquals(1, s.disposedCount.get());
    }
    ...
    public void workerNotDisposedPrematurelyAsyncInNormalOutConditional() {
        ...
        up.onNext(1);
        up.onComplete();
        Flowable.concat(up.observeOn(s).filter(Functions.alwaysTrue()), Flowable.just(2)).test().assertResult(1, 2);
        assertEquals(1, s.disposedCount.get());
    }
    ...
    public void fusedNoConcurrentCleanDueToCancel() {
        for (int j = 0; j < TestHelper.RACE_LONG_LOOPS; j++) {
            ...
            try {
                final UnicastProcessor<Integer> up = UnicastProcessor.create();
                TestObserver<Integer> to = up.hide().observeOn(Schedulers.io()).observeOn(Schedulers.single()).unsubscribeOn(Schedulers.computation()).firstOrError().test();
    ...
    public void syncFusedCancelAfterPoll() {
        ...
        Flowable.just(1).map(v -> {
            ts.cancel();
            return v + 1;
        }).compose(TestHelper.flowableStripBoundary()).observeOn(ImmediateThinScheduler.INSTANCE).subscribe(ts);
        ts.assertEmpty();
    }
    ...
    public void syncFusedCancelAfterPollConditional() {
        ...
        Flowable.just(1).map(v -> {
            ts.cancel();
            return v + 1;
        }).compose(TestHelper.flowableStripBoundary()).observeOn(ImmediateThinScheduler.INSTANCE).compose(TestHelper.conditional()).subscribe(ts);
        ts.assertEmpty();
    }
    ...
    public void backFusedMoreWork() {
        ...
        pp.observeOn(ImmediateThinScheduler.INSTANCE).doOnNext(v -> {
            if (v == 1) {
                pp.onNext(2);
            }
        }).subscribe(ts);
    ...
    public void backFusedConditionalMoreWork() {
        ...
        }).compose(TestHelper.conditional()).subscribe(ts);
        pp.onNext(1);
        ts.assertValuesOnly(1, 2);
    }
