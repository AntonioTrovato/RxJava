...
public class ObservableMergeWithMaybeTest extends RxJavaTest {
    ...
    public void onErrorOtherOverflow() {
        List<Throwable> errors = TestHelper.trackPluginErrors();
        try {
            Observable.error(new IOException()).mergeWith(Maybe.error(new TestException())).test().assertFailure(IOException.class);
            TestHelper.assertUndeliverable(errors, 0, TestException.class);
        } finally {
            RxJavaPlugins.reset();
        }
    ...
    public void isDisposed() {
        ...
        }.mergeWith(Maybe.<Integer>empty()).take(1).test().assertResult(1);
    }
