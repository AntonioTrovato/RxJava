...
public class FlowableCombineLatestTest extends RxJavaTest {
    ...
    public void nonFatalExceptionThrownByCombinatorForSingleSourceIsNotReportedByUpstreamOperator() {
        ...
        Flowable<Integer> source = Flowable.just(1).// if haven't caught exception in combineLatest operator then would incorrectly
    ...
    public void combineLatestArrayOfSources() {
        Flowable.combineLatestArray(new Flowable[] { Flowable.just(1), Flowable.just(2) }, new Function<Object[], Object>() {
    ...
    public void combineLatestDelayErrorArrayOfSources() {
        Flowable.combineLatestArrayDelayError(new Flowable[] { Flowable.just(1), Flowable.just(2) }, new Function<Object[], Object>() {
    ...
    public void combineLatestDelayErrorArrayOfSourcesWithError() {
        Flowable.combineLatestArrayDelayError(new Flowable[] { Flowable.just(1), Flowable.just(2).concatWith(Flowable.<Integer>error(new TestException())) }, new Function<Object[], Object>() {
    ...
    public void combineLatestDelayErrorIterableOfSources() {
        Flowable.combineLatestDelayError(Arrays.asList(Flowable.just(1), Flowable.just(2)), new Function<Object[], Object>() {
    ...
    public void errorDelayed() {
        Flowable.combineLatestArrayDelayError(new Publisher[] { Flowable.error(new TestException()), Flowable.just(1) }, new Function<Object[], Object>() {
    ...
    public void errorDelayed2() {
        Flowable.combineLatestArrayDelayError(new Publisher[] { Flowable.error(new TestException()).startWithItem(1), Flowable.empty() }, new Function<Object[], Object>() {
    ...
    public void dontSubscribeIfDone2() {
        ...
        try {
            final int[] count = { 0 };
            Flowable.combineLatestDelayError(Arrays.asList(Flowable.empty(), Flowable.error(new TestException()).doOnSubscribe(new Consumer<Subscription>() {
    ...
    public void fusedNullCheck() {
        ...
        ts.assertFusionMode(QueueFuseable.ASYNC).assertFailureAndMessage(NullPointerException.class, "The combiner returned a null value");
    }

    @Test
    public void syncFirstErrorsAfterItemDelayError() {
        Flowable.combineLatestDelayError(Arrays.asList(Flowable.just(21).concatWith(Flowable.<Integer>error(new TestException())), Flowable.just(21).delay(100, TimeUnit.MILLISECONDS)), new Function<Object[], Object>() {
    ...
    public void onCompleteDisposeRace() {
        for (int i = 0; i < TestHelper.RACE_DEFAULT_LOOPS; i++) {
            ...
            Flowable.combineLatest(pp, Flowable.never(), (a, b) -> a).subscribe(ts);
    ...
    public void doneButNotEmpty() {
        ...
        TestSubscriber<Integer> ts = Flowable.combineLatest(pp1, pp2, (a, b) -> a + b).doOnNext(v -> {
            if (v == 2) {
                pp2.onNext(3);
                pp2.onComplete();
                pp1.onComplete();
    ...
    public void syncFusionRejected() {
        ...
        Flowable.combineLatest(Flowable.never(), Flowable.never(), (a, b) -> a).subscribe(ts);
    ...
    public void fusedNormal() {
        ...
        Flowable.combineLatest(Flowable.just(1), Flowable.just(2), (a, b) -> a + b).subscribeWith(ts).assertFuseable().assertFusionMode(QueueFuseable.ASYNC).assertResult(3);
    }
    ...
    public void fusedToParallel() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<>();
        ts.setInitialFusionMode(QueueFuseable.ANY);
        Flowable.combineLatest(Flowable.just(1), Flowable.just(2), (a, b) -> a + b).parallel().sequential().subscribeWith(ts).assertResult(3);
    }
    ...
    public void fusedToParallel2() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<>();
        ts.setInitialFusionMode(QueueFuseable.ANY);
        Flowable.combineLatest(Flowable.just(1), Flowable.just(2), (a, b) -> a + b).compose(TestHelper.flowableStripBoundary()).parallel().sequential().subscribeWith(ts).assertResult(3);
    }
    ...
    public void fusedError() {
        TestSubscriberEx<Integer> ts = new TestSubscriberEx<>();
        ts.setInitialFusionMode(QueueFuseable.ANY);
        Flowable.combineLatest(Flowable.just(1), Flowable.<Integer>error(new TestException()), (a, b) -> a + b).subscribeWith(ts).assertFuseable().assertFusionMode(QueueFuseable.ASYNC).assertFailure(TestException.class);
    }

    @Test
    public void nonFusedMoreWorkBeforeTermination() {
        PublishProcessor<Integer> pp = PublishProcessor.create();
        TestSubscriber<Integer> ts = Flowable.combineLatest(pp, Flowable.just(1), (a, b) -> a + b).doOnNext(v -> {
            if (v == 1) {
                pp.onNext(2);
                pp.onComplete();
            }
    ...
    public void nonFusedDelayErrorMoreWorkBeforeTermination() {
        PublishProcessor<Integer> pp = PublishProcessor.create();
        TestSubscriber<List<Object>> ts = Flowable.combineLatestDelayError(Arrays.asList(pp, Flowable.just(1)), a -> Arrays.asList(a)).doOnNext(v -> {
    ...
    public void fusedCombinerCrashError() {
        TestSubscriberEx<Object> ts = new TestSubscriberEx<>();
        ts.setInitialFusionMode(QueueFuseable.ANY);
        Flowable.combineLatest(Flowable.just(1), Flowable.just(1), (a, b) -> {
            throw new TestException();
        }).subscribeWith(ts).assertFuseable().assertFusionMode(QueueFuseable.ASYNC).assertFailure(TestException.class);
    }
    ...
    public void fusedCombinerCrashError2() {
        Flowable.combineLatest(Flowable.just(1), Flowable.just(1), (a, b) -> {
            throw new TestException();
        }).compose(TestHelper.flowableStripBoundary()).rebatchRequests(10).test().assertFailure(TestException.class);
    }
}
