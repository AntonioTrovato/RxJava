...
public class FlowableThrottleLatestTest extends RxJavaTest {
    ...
    public void onDroppedBasicNoEmitLast() {
        ...
        TestSubscriber<Integer> ts = pp.throttleLatest(1, TimeUnit.SECONDS, sch, false, drops::onNext).test();
        ts.assertEmpty();
        drops.assertEmpty();
        pp.onNext(1);
        ts.assertValuesOnly(1);
    ...
    public void onDroppedBasicEmitLast() {
        ...
        TestSubscriber<Object> drops = new TestSubscriber<>();
        drops.onSubscribe(EmptySubscription.INSTANCE);
        TestSubscriber<Integer> ts = pp.throttleLatest(1, TimeUnit.SECONDS, sch, true, drops::onNext).test();
        ts.assertEmpty();
        drops.assertEmpty();
        pp.onNext(1);
        ts.assertValuesOnly(1);
    ...
    public void onDroppedBasicNoEmitLastFirstDropCrash() throws Throwable {
        ...
        TestSubscriber<Integer> ts = pp.doOnCancel(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, d -> {
            if (d == 2) {
    ...
    public void onDroppedBasicNoEmitLastOnErrorDropCrash() throws Throwable {
        ...
        TestScheduler sch = new TestScheduler();
        Action whenDisposed = mock(Action.class);
        TestSubscriberEx<Integer> ts = pp.doOnCancel(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, d -> {
            throw new TestException("forced " + d);
    ...
    public void onDroppedBasicEmitLastOnErrorDropCrash() throws Throwable {
        ...
        TestScheduler sch = new TestScheduler();
        Action whenDisposed = mock(Action.class);
        TestSubscriberEx<Integer> ts = pp.doOnCancel(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, true, d -> {
            throw new TestException("forced " + d);
        }).subscribeWith(new TestSubscriberEx<>());
        ts.assertEmpty();
        pp.onNext(1);
    ...
    public void onDroppedDisposeDrops() throws Throwable {
        ...
        TestSubscriber<Object> drops = new TestSubscriber<>();
        drops.onSubscribe(EmptySubscription.INSTANCE);
        TestSubscriberEx<Integer> ts = pp.doOnCancel(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, drops::onNext).subscribeWith(new TestSubscriberEx<>());
        ts.assertEmpty();
        pp.onNext(1);
        ts.assertValuesOnly(1);
        pp.onNext(2);
    ...
    public void onDroppedBackpressured() throws Throwable {
        ...
        drops.onSubscribe(EmptySubscription.INSTANCE);
        Action whenDisposed = mock(Action.class);
        TestSubscriber<Integer> ts = pp.doOnCancel(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, drops::onNext).test(0L);
        ts.assertEmpty();
        drops.assertEmpty();
        pp.onNext(1);
    ...
    public void onDroppedBackpressuredDropCrash() throws Throwable {
        ...
        TestHelper.assertCompositeExceptions(ts, MissingBackpressureException.class, "Could not emit value due to lack of requests", TestException.class, "forced 1");
        verify(whenDisposed).run();
    }
}
