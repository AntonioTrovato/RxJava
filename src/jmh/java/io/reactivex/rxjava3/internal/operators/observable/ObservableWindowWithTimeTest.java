...
public class ObservableWindowWithTimeTest extends RxJavaTest {
    ...
    public void exactWindowSize() {
        Observable<Observable<Integer>> source = Observable.range(1, 10).window(1, TimeUnit.MINUTES, scheduler, 3);
    ...
    public void cancellingWindowCancelsUpstreamExactTime() {
        PublishSubject<Integer> ps = PublishSubject.create();
        TestObserver<Integer> to = ps.window(10, TimeUnit.MINUTES).take(1).flatMap(new Function<Observable<Integer>, Observable<Integer>>() {
    ...
    public void windowAbandonmentCancelsUpstreamExactTime() {
        ...
        TestObserver<Observable<Integer>> to = ps.window(10, TimeUnit.MINUTES).take(1).doOnNext(new Consumer<Observable<Integer>>() {
    ...
    public void cancellingWindowCancelsUpstreamExactTimeAndSize() {
        PublishSubject<Integer> ps = PublishSubject.create();
        TestObserver<Integer> to = ps.window(10, TimeUnit.MINUTES, 100).take(1).flatMap(new Function<Observable<Integer>, Observable<Integer>>() {
    ...
    public void windowAbandonmentCancelsUpstreamExactTimeAndSize() {
        PublishSubject<Integer> ps = PublishSubject.create();
        final AtomicReference<Observable<Integer>> inner = new AtomicReference<>();
        TestObserver<Observable<Integer>> to = ps.window(10, TimeUnit.MINUTES, 100).take(1).doOnNext(new Consumer<Observable<Integer>>() {
    ...
    public void cancellingWindowCancelsUpstreamExactTimeSkip() {
        PublishSubject<Integer> ps = PublishSubject.create();
        TestObserver<Integer> to = ps.window(10, 15, TimeUnit.MINUTES).take(1).flatMap(new Function<Observable<Integer>, Observable<Integer>>() {
    ...
    public void windowAbandonmentCancelsUpstreamExactTimeSkip() {
        PublishSubject<Integer> ps = PublishSubject.create();
        final AtomicReference<Observable<Integer>> inner = new AtomicReference<>();
        TestObserver<Observable<Integer>> to = ps.window(10, 15, TimeUnit.MINUTES).take(1).doOnNext(new Consumer<Observable<Integer>>() {
    ...
    public void timedBoundarySignalAndDisposeRace() {
        for (int i = 0; i < TestHelper.RACE_DEFAULT_LOOPS; i++) {
            TestScheduler scheduler = new TestScheduler();
            PublishSubject<Integer> ps = PublishSubject.create();
            TestObserver<Observable<Integer>> to = ps.window(1, TimeUnit.MINUTES, scheduler, 1).test();
            TestHelper.race(() -> ps.onNext(1), () -> to.dispose());
        }
    }
}
