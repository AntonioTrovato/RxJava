...
public class FlowableRefCountTest extends RxJavaTest {
    ...
    void refCountAsyncActual() {
        ...
        // give time to emit
        // now unsubscribe
        // unsubscribe s2 first as we're counting in 1 and there can be a race between unsubscribe and one subscriber getting a value but not the other
        d2.dispose();
        d1.dispose();
    ...
    public void replayNoLeak2() throws Exception {
        ...
        }).concatWith(Flowable.never()).replay(1).refCount();
    ...
    public void replayIsUnsubscribed() {
        ConnectableFlowable<Integer> cf = Flowable.just(1).replay();
    ...
    public void comeAndGo() {
        PublishProcessor<Integer> pp = PublishProcessor.create();
        Flowable<Integer> source = pp.publish().refCount(1);
        TestSubscriber<Integer> ts1 = source.test(0);
        assertTrue(pp.hasSubscribers());
        for (int i = 0; i < 3; i++) {
            TestSubscriber<Integer> ts2 = source.test();
    ...
    public void unsubscribeSubscribeRace() {
        for (int i = 0; i < 1000; i++) {
            final Flowable<Integer> source = Flowable.range(1, 5).replay().refCount(1);
            ...
            // FIXME RxJava replay() doesn't issue onComplete without request
            ts2.requestMore(6).withTag("Round: " + i).assertResult(1, 2, 3, 4, 5);
        }
    }

    static final class BadFlowableDoubleOnX extends ConnectableFlowable<Object> implements Disposable {
    ...
    public void doubleOnX() {
        List<Throwable> errors = TestHelper.trackPluginErrors();
        try {
            new BadFlowableDoubleOnX().refCount().test().assertResult();
    ...
    public void doubleOnXCount() {
        List<Throwable> errors = TestHelper.trackPluginErrors();
        try {
            new BadFlowableDoubleOnX().refCount(1).test().assertResult();
            TestHelper.assertError(errors, 0, ProtocolViolationException.class);
            TestHelper.assertUndeliverable(errors, 1, TestException.class);
        } finally {
            RxJavaPlugins.reset();
    ...
    public void doubleOnXTime() {
        List<Throwable> errors = TestHelper.trackPluginErrors();
        try {
            new BadFlowableDoubleOnX().refCount(5, TimeUnit.SECONDS, Schedulers.single()).test().assertResult();
            TestHelper.assertError(errors, 0, ProtocolViolationException.class);
            TestHelper.assertUndeliverable(errors, 1, TestException.class);
        } finally {
            RxJavaPlugins.reset();
    ...
    public void cancelTerminateStateExclusion() {
        FlowableRefCount<Object> o = (FlowableRefCount<Object>) PublishProcessor.create().publish().refCount();
        o.cancel(null);
    ...
    public void publishRefCountShallBeThreadSafe() {
        for (int i = 0; i < TestHelper.RACE_LONG_LOOPS; i++) {
            ...
            TestSubscriber<Integer> subscriber1 = flowable.subscribeOn(Schedulers.io()).test();
            TestSubscriber<Integer> subscriber2 = flowable.subscribeOn(Schedulers.io()).test();
            subscriber1.withTag("subscriber1 " + i).awaitDone(5, TimeUnit.SECONDS).assertNoErrors().assertComplete();
            subscriber2.withTag("subscriber2 " + i).awaitDone(5, TimeUnit.SECONDS).assertNoErrors().assertComplete();
        }
    }
