...
public class SingleFlatMapIterableFlowableTest extends RxJavaTest {
    ...
    public void slowPatchCancelAfterOnNext() {
        ...
        Single.just(1).flattenAsFlowable(v -> Arrays.asList(1, 2)).subscribe(ts);
        ts.assertResult(1);
    }
    ...
    public void onSuccessRequestRace() {
        ...
        for (int i = 0; i < TestHelper.RACE_DEFAULT_LOOPS; i++) {
            ...
            TestSubscriber<Object> ts = ss.flattenAsFlowable(v -> list).test(0L);
            TestHelper.race(() -> ss.onSuccess(1), () -> ts.request(1));
            ts.assertResult(1);
        }
    }
}
