...
public class ObservableThrottleLatestTest extends RxJavaTest {
    ...
    public void onDroppedBasicNoEmitLast() {
        ...
        TestObserver<Integer> to = ps.throttleLatest(1, TimeUnit.SECONDS, sch, false, drops::onNext).test();
        to.assertEmpty();
        drops.assertEmpty();
        ps.onNext(1);
        to.assertValuesOnly(1);
    ...
    public void onDroppedBasicEmitLast() {
        ...
        TestObserver<Object> drops = new TestObserver<>();
        drops.onSubscribe(Disposable.empty());
        TestObserver<Integer> to = ps.throttleLatest(1, TimeUnit.SECONDS, sch, true, drops::onNext).test();
        to.assertEmpty();
        drops.assertEmpty();
        ps.onNext(1);
        to.assertValuesOnly(1);
    ...
    public void onDroppedBasicNoEmitLastFirstDropCrash() throws Throwable {
        ...
        TestObserver<Integer> to = ps.doOnDispose(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, d -> {
            if (d == 2) {
    ...
    public void onDroppedBasicNoEmitLastOnErrorDropCrash() throws Throwable {
        ...
        TestScheduler sch = new TestScheduler();
        Action whenDisposed = mock(Action.class);
        TestObserverEx<Integer> to = ps.doOnDispose(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, d -> {
    ...
    public void onDroppedBasicEmitLastOnErrorDropCrash() throws Throwable {
        ...
        TestScheduler sch = new TestScheduler();
        Action whenDisposed = mock(Action.class);
        TestObserverEx<Integer> to = ps.doOnDispose(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, true, d -> {
            throw new TestException("forced " + d);
        }).subscribeWith(new TestObserverEx<>());
        to.assertEmpty();
        ps.onNext(1);
    ...
    public void onDroppedDisposeDrops() throws Throwable {
        ...
        TestObserver<Object> drops = new TestObserver<>();
        drops.onSubscribe(Disposable.empty());
        TestObserverEx<Integer> to = ps.doOnDispose(whenDisposed).throttleLatest(1, TimeUnit.SECONDS, sch, false, drops::onNext).subscribeWith(new TestObserverEx<>());
        to.assertEmpty();
        ps.onNext(1);
        to.assertValuesOnly(1);
        ps.onNext(2);
