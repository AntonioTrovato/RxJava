...
public class ObservableObserveOnTest extends RxJavaTest {
    ...
    public void delayError() {
        Observable.range(1, 5).concatWith(Observable.<Integer>error(new TestException())).observeOn(Schedulers.computation(), true).doOnNext(new Consumer<Integer>() {
    ...
    public void outputFused() {
        ...
        Observable.range(1, 5).hide().observeOn(Schedulers.single()).subscribe(to);
        to.assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2, 3, 4, 5);
    }
    ...
    public void outputFusedReject() {
        ...
        Observable.range(1, 5).hide().observeOn(Schedulers.single()).subscribe(to);
        to.assertFusionMode(QueueFuseable.NONE).awaitDone(5, TimeUnit.SECONDS).assertResult(1, 2, 3, 4, 5);
    }
    ...
    public void inputOutputAsyncFusedError() {
        ...
        us.onError(new TestException());
        to.awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
        to.assertFusionMode(QueueFuseable.ASYNC).awaitDone(5, TimeUnit.SECONDS).assertFailure(TestException.class);
    }
    ...
    static final class TestObserverFusedCanceling extends TestObserverEx<Integer> {
    ...
    public void fusedNoConcurrentCleanDueToCancel() {
        for (int j = 0; j < TestHelper.RACE_LONG_LOOPS; j++) {
            ...
            try {
                final UnicastSubject<Integer> us = UnicastSubject.create();
                TestObserver<Integer> to = us.hide().observeOn(Schedulers.io()).observeOn(Schedulers.single()).unsubscribeOn(Schedulers.computation()).firstOrError().test();
