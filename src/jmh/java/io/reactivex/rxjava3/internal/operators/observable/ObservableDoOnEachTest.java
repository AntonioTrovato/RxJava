...
public class ObservableDoOnEachTest extends RxJavaTest {
    ...
    public void onErrorOnErrorCrashConditional() {
        TestObserverEx<Object> to = Observable.error(new TestException("Outer")).doOnError(new Consumer<Throwable>() {
    ...
    @Test
    // TODO decide/implement fusion
    @Ignore("Fusion not supported yet")
    public void fused() {
    ...
    public void fusedOnErrorCrash() {
        ...
        to.assertFuseable().assertFusionMode(QueueFuseable.SYNC).assertFailure(TestException.class);
    ...
    public void fusedAsync() {
        ...
        us.doOnNext(new Consumer<Integer>() {
    ...
    public void fusedAsyncConditional2() {
        ...
        final int[] call = { 0, 0 };
        UnicastSubject<Integer> us = UnicastSubject.create();
        us.hide().doOnNext(new Consumer<Integer>() {
        ...
        TestHelper.emit(us, 1, 2, 3, 4, 5);
        to.assertFuseable().assertFusionMode(QueueFuseable.NONE).assertResult(1, 2, 3, 4, 5);
        assertEquals(5, call[0]);
        assertEquals(1, call[1]);
    }
