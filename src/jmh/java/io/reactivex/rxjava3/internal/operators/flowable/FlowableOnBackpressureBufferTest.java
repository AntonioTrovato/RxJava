...
public class FlowableOnBackpressureBufferTest extends RxJavaTest {
    ...
    public void onDroppedNormal() throws Throwable {
        ...
        TestSubscriber<Integer> ts = pp.onBackpressureBuffer(1, false, false, () -> {
        }, onDropped).test(0L);
        ts.assertEmpty();
        pp.onNext(1);
        ts.assertEmpty();
    ...
    public void onDroppedCrash() throws Throwable {
        ...
        TestSubscriberEx<Integer> ts = pp.onBackpressureBuffer(1, false, false, () -> {
        }, onDropped).subscribeWith(new TestSubscriberEx<Integer>(0L));
        ts.assertEmpty();
        pp.onNext(1);
        ts.assertEmpty();
        pp.onNext(2);
