...
public class FlowableUnsubscribeOnTest extends RxJavaTest {
    ...
    public void takeHalf() {
        int elements = 1024;
        Flowable.range(0, elements * 2).unsubscribeOn(Schedulers.single()).take(elements).to(TestHelper.<Integer>testConsumer()).awaitDone(5, TimeUnit.SECONDS).assertValueCount(elements).assertComplete().assertNoErrors().assertSubscribed();
    }
