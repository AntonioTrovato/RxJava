...
public class FlowableUsingTest extends RxJavaTest {
    ...
    private void performTestUsing(boolean disposeEagerly) {
        ...
        Flowable<String> flowable = Flowable.using(resourceFactory, observableFactory, new DisposeAction(), disposeEagerly);
    ...
    public void usingDoesNotDisposesEagerlyBeforeCompletion() {
        ...
        Subscriber<String> subscriber = TestHelper.mockSubscriber();
        Flowable<String> flowable = Flowable.using(resourceFactory, observableFactory, new DisposeAction(), false).doOnCancel(unsub).doOnComplete(completion);
        flowable.safeSubscribe(subscriber);
    ...
    public void usingDoesNotDisposesEagerlyBeforeError() {
        ...
        Subscriber<String> subscriber = TestHelper.mockSubscriber();
        Flowable<String> flowable = Flowable.using(resourceFactory, observableFactory, new DisposeAction(), false).doOnCancel(unsub).doOnError(onError);
        flowable.safeSubscribe(subscriber);
    ...
    public void factoryThrows() {
        ...
        Flowable.<Integer, Integer>using(new Supplier<Integer>() {
    ...
    @Test
    public void dispose() {
        TestHelper.checkDisposed(Flowable.using(new Supplier<Object>() {
    ...
    public void eagerDisposeResourceThenDisposeUpstream() {
        ...
        Flowable.using(Functions.justSupplier(1), new Function<Integer, Flowable<Integer>>() {

            @Override
            public Flowable<Integer> apply(Integer t) throws Throwable {
                return Flowable.range(1, 2).doOnCancel(new Action() {
