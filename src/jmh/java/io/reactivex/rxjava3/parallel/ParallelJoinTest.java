...
public class ParallelJoinTest extends RxJavaTest {
    ...
    public void overflowFastpath() {
        ...
        }.sequential(1).test(0).assertFailure(QueueOverflowException.class);
    }
    ...
    public void overflowFastpathDelayError() {
        ...
        }.sequentialDelayError(1).test(0).requestMore(1).assertFailure(QueueOverflowException.class, 1);
    }
    ...
    public void delayError() {
        TestSubscriberEx<Integer> flow = Flowable.range(1, 2).parallel(2).map(new Function<Integer, Integer>() {

            @Override
            public Integer apply(Integer v) throws Exception {
                throw new TestException();
            }
        }).sequentialDelayError().to(TestHelper.<Integer>testConsumer()).assertFailure(CompositeException.class);
    ...
    public void delayErrorCancelBackpressured() {
        TestSubscriber<Integer> ts = Flowable.range(1, 3).parallel(1).sequentialDelayError(1).test(0);
        ts.cancel();
        ts.assertEmpty();
    }
    ...
    public void delayErrorCancelBackpressured2() {
        TestSubscriber<Integer> ts = Flowable.<Integer>empty().parallel(1).sequentialDelayError(1).test(0);
        ts.assertResult();
    }
    ...
    public void failedRailIsIgnored() {
        Flowable.range(1, 4).parallel(2).map(new Function<Integer, Integer>() {
    ...
    public void failedRailIsIgnoredHidden() {
        Flowable.range(1, 4).hide().parallel(2).map(new Function<Integer, Integer>() {
    ...
    public void oneItemNext() {
        ...
        TestSubscriber<Integer> ts = pp.parallel(1).sequential().test(0L);
        pp.onNext(1);
        ts.requestMore(10).assertValuesOnly(1);
    }

    @Test
    public void delayErrorOneItemNext() {
        PublishProcessor<Integer> pp = PublishProcessor.create();
        TestSubscriber<Integer> ts = pp.parallel(1).sequentialDelayError().test(0L);
        pp.onNext(1);
        ts.requestMore(10).assertValuesOnly(1);
    }
    ...
    public void onNextWhileProcessingSlowPath() {
        ...
        ParallelFlowable.fromArray(pp).sequential().subscribeWith(ts);
        pp.onNext(1);
    ...
    public void delayErrorOnNextWhileProcessingSlowPath() {
        ...
        ParallelFlowable.fromArray(pp).sequentialDelayError().subscribeWith(ts);
        pp.onNext(1);
        ts.assertValuesOnly(1, 2);
    }
