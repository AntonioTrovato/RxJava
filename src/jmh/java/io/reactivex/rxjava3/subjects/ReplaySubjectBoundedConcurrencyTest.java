...
public class ReplaySubjectBoundedConcurrencyTest extends RxJavaTest {
    ...
    public void replaySubjectEmissionSubscriptionRace() throws Exception {
        ...
        try {
            for (int i = 0; i < 50000; i++) {
                ...
                rs.// .doOnSubscribe(v -> System.out.println("!! " + j))
                // .doOnNext(e -> System.out.println(">> " + j))
                subscribeOn(s).observeOn(Schedulers.io()).// .doOnNext(e -> System.out.println(">>> " + j))
    ...
    public void concurrentSizeAndHasAnyValueBounded() throws InterruptedException {
        ...
        for (; !rs.hasThrowable() && !rs.hasComplete(); ) {
            // can't use value so just call to detect hangs
            rs.size();
            // can't use value so just call to detect hangs
            rs.hasValue();
            Object[] values = rs.getValues();
